...
var name = User(name: "joe", age: 17, height: 71, weight: 185, activityLevel: 8)
print("\(name.name) is \(name.age), he is \(name.height) inch's tall and weighs \(name.weight) and is fairly active at a \(name.activityLevel) out of 10")
/*:
...
struct Distance {
    ...
    init(feet:Double) {
        self.feet = feet
        meters = self.feet / 3.28084
    }
    
    init(meters: Double) {
        self.meters = meters
        feet = self.meters * 3.28084
    }
}
