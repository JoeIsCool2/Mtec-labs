...
let joe = GetUserRequest(username: "Joe")
let workoutService = WorkoutAPIService()

Task {
    try await workoutService.performRequest(joe)
}

//It will be type User
...
let recentWorkoutRequest = GetRecentWorkoutRequest()
Task {
    let workout = try await workoutService.performRequest(recentWorkoutRequest)
    // workout is type Workout
}
